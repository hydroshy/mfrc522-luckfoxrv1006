...
class MFRC522:
    ...
    def __init__(self, bus=0, device=0, spd=1000000, pin_rst=17, debugLevel='WARNING'):
        self.spi = SPI("/dev/spidev0.0", device, spd)  # Sử dụng python-periphery cho SPI
        self.logger = logging.getLogger('mfrc522Logger')
        self.logger.addHandler(logging.StreamHandler())
        level = logging.getLevelName(debugLevel)
        self.logger.setLevel(level)

        self.rst = GPIO(pin_rst, "out")  # Sử dụng python-periphery cho GPIO
        self.rst.write(True)  # Reset the RC522 module
        self.MFRC522_Init()
    ...
    def Write_MFRC522(self, addr, val):
        self.spi.transfer([(addr << 1) & 0x7E, val])  # Điều chỉnh hàm xfer2

    def Read_MFRC522(self, addr):
        val = self.spi.transfer([((addr << 1) & 0x7E) | 0x80, 0])  # Điều chỉnh hàm xfer2
        return val[1]

    def Close_MFRC522(self):
        self.spi.close()
        self.rst.close()  # Đóng GPIO
